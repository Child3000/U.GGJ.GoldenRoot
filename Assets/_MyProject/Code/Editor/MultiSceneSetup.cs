...
public static class MultiSceneSetupMenu
{
    ...
    public static bool LoadMultisceneSetupOnDoubleClick(int instanceID, int line)
    {
        var obj = EditorUtility.InstanceIDToObject(instanceID) as MultiSceneSetup;
        if (obj != null) {
            string name = obj.name;

            EditorSceneManager.RestoreSceneManagerSetup(obj.Setups);
            Debug.Log(string.Format("Scene setup '{0}' restored", name));
            return true;
        }
    ...
    static void SaveCurrentSceneSetup(string assetPath)
    {
        ...
        Debug.Log(string.Format("Scene setup '{0}' saved", 
    ...
    public static void RestoreSceneSetup()
    {
        ...
        Debug.Log(string.Format("Scene setup '{0}' restored", 
            Path.GetFileNameWithoutExtension(assetPath)));
    }
